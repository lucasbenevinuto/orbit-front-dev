...
function TranscriptionTracker({ upload }: { upload: { id: string; original_filename: string; status: string; error_message: string | null } }) {
  // Determine current step from audio status
...
export function UploadPage() {
  ...
  const handleUpload = useCallback(async (file: File) => {
    ...
    try {
      ...
      await uploadFileToS3(result.upload_url, file, (progress) => {
      ...
      try {
        ...
        toast.error('Upload OK, mas erro ao iniciar transcrição automática')
      }
    } catch {
  ...
  const handleDeleteUpload = async (id: string, filename: string) => {
    ...
    try {
      await deleteAudioUpload.mutateAsync(id)
  ...
  const statusConfig = {
    uploading: { label: 'Enviando', icon: Clock, className: 'text-yellow-600 bg-yellow-500/10' },
    ...
    transcribed: { label: 'Transcrito', icon: CheckCircle2, className: 'text-green-600 bg-green-500/10' },
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Upload de Áudio</h1>
          ...
          <div className="rounded-lg border border-border bg-card p-4 text-right">
            ...
            <div className="mt-2 h-1.5 w-32 overflow-hidden rounded-full bg-muted">
              <div
                ...
                style={{
                  width: `${Math.min(((user.transcription_credits_used_cents || 0) / user.transcription_credits_limit_cents!) * 100, 100)}%`,
                }}
              />
            </div>
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          ...
          <select
            ...
            className="flex h-10 w-full max-w-md rounded-md border border-input bg-background pl-9 pr-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
          >
        ...
        {projects.length === 0 && (
          <p className="text-xs text-muted-foreground">
            Nenhum projeto encontrado. Crie um projeto primeiro em Projetos.
          </p>
        )}
      </div>

      {/* Diarization toggle */}
      <div className="flex items-center justify-between rounded-lg border border-border bg-card p-4 max-w-md">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {enableDiarization
              ? 'Usa IA avançada para separar falas por participante (mais lento e caro)'
              : 'Transcrição rápida sem separação de participantes'}
          </p>
        </div>
        <button
        ...
        {uploadState === 'idle' ? (
          <>
            ...
            <p className="mt-2 text-xs text-muted-foreground">
              Formatos: {ALLOWED_EXTENSIONS.join(', ')} | Máx: {formatFileSize(MAX_SIZE)}
            </p>
            {selectedProjectId && (
              <input
                type="file"
      ...
      {(() => {
        const processingUploads = uploads?.data.filter(
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {formatFileSize(upload.file_size_bytes)} &middot;{' '}
